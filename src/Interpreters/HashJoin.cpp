...
bool HashJoin::addJoinedBlock(const Block & source_block, bool check_limits)
{
...

    {
        ...
        for (size_t d = 0; d < disjuncts_num; ++d)
        {
            ...
            std::transform(std::cbegin(key_names_right_indexes[d]),
                           std::cend(key_names_right_indexes[d]),
                           std::begin(key_columns),
                           [&all_key_columns](size_t i){ return all_key_columns[i]; });
...
BlockInputStreamPtr HashJoin::createStreamWithNonJoinedRows(const Block & result_sample_block, UInt64 max_block_size) const
{
    ...
    if (isRightOrFull(table_join->kind()))
    {
        ...
        if (multiple_disjuncts)
        {
            return std::make_shared<NonJoinedBlockInputStream<true>>(*this, result_sample_block, max_block_size);
        }
        else
        {
            return std::make_shared<NonJoinedBlockInputStream<false>>(*this, result_sample_block, max_block_size);
        }
    }
    return {};
}
